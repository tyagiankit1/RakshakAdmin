...
export const getQRLotList = async (infState) => {
    setHeader(localStorage.accessToken)
    const response = await axios.post(infState.api+'getQRLotList')
    console.log("response service: ", response)
    return response.data;
}

export const createQRLot = async (infState, payload) => {
    setHeader(localStorage.accessToken)
    const response = await axios.post(infState.api+'createQRLot',  payload);
    console.log("response service: ", response)
    return response.data;
}

export const generateQRLot = async (infState, payload) => {
    setHeader(localStorage.accessToken)
    const response = await axios.post(infState.api+'generateQRLot', payload);
    console.log("response service: ", response)
    return response.data;
}

export const downloadQRLot = async (infState, payload) => {
    setHeader(localStorage.accessToken)
    const response = await axios.post(infState.api+'downloadQRLot', payload);
