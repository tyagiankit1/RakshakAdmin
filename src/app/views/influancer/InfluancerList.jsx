...
const InfluancerList = () => {
    ...
    useEffect(() => {
        getInfluancerList(infState).then(( data ) => {
               ...
               <Table className="whitespace-pre">
                <TableHead>
                    <TableRow>
                        ...
                        <TableCell className="px-0">Platform</TableCell>
                        <TableCell className="px-0">Followers</TableCell>
                    ...
                    {influancerDataList
                        ...
                        .map((influancerData, index) => (
                            <TableRow key={index}>
                                ...
                                <TableCell className="px-0 capitalize">
                                    {influancerData.platform}
                                </TableCell>
                                <TableCell className="px-0 capitalize">
                                    {influancerData.followers}
                                </TableCell>
                                <TableCell className="px-0">
                                    {
                                        influancerData.status === 'Draft' ? <small className="border-radius-4 bg-error text-white px-2 py-2px">{influancerData.status}</small>
                                        ...
                                        : influancerData.status === 'Pending' ? <small className="border-radius-4 bg-primary text-white px-2 py-2px">{influancerData.status}</small>
                                        : influancerData.status === 'Active' ? <small className="border-radius-4 bg-green text-white px-2 py-2px">{influancerData.status}</small>
                                        : influancerData.status === 'In-Active' ? <small className="border-radius-4 bg-error text-white px-2 py-2px">{influancerData.status}</small>
                                        :''
                                    }
                                </TableCell>
                            </TableRow>
